...
import { ArrowRight, Shield, TrendingDown, Target } from 'lucide-react';
...
function ReportPreview({ accent, isFa }: { accent: string; isFa: boolean }) {
  const radius = 46;
  ...
  return (
    ...
    >
      {/* Subtle continuous float — respects prefers-reduced-motion via the
          global rule in RedesignHomePage's <style> block. */}
      <motion.div
        animate={{ y: [0, -6, 0] }}
...
export function Hero({ copy, chrome }: { copy: HeroCopy; chrome: SectionChrome }) {
  const { accent, isFa, displayFont } = chrome;

  return (
    <section className="max-w-screen-2xl mx-auto px-6 pt-20 pb-16 grid md:grid-cols-[1.1fr_0.9fr] gap-14 items-center">
      ...
      <div className={`flex items-center justify-center ${isFa ? 'md:order-1' : ''}`}>
        <ReportPreview accent={accent} isFa={isFa} />
      </div>
    </section>
  );
}
